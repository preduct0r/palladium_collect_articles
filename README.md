...
Этот проект представляет собой полный pipeline для работы с научными статьями: от сбора PDF по графу цитирований до индексации в векторную базу данных для семантического поиска.

## Полный Pipeline обработки

Система состоит из 4 последовательных этапов:

### 1. Сбор статей (`main.py`)
- **Вход:** DOI статьи
- **Процесс:** 
  - Получает метаданные из OpenAlex API
  - Скачивает PDF (OpenAlex или Sci-Hub)
  - Обходит граф цитирований (BFS)
- **Выход:**
  - `articles.db` (SQLite) - метаданные и связи между статьями
  - S3 bucket `palladium-articles/` - PDF файлы

### 2. Парсинг PDF в Markdown (`pdf_parser.py`)
- **Вход:** PDF из S3 (`palladium-articles`)
- **Процесс:**
  - Использует marker library
  - Конвертирует PDF → Markdown
  - Извлекает текст, формулы, таблицы
- **Выход:**
  - S3 bucket `palladium-pdf-to-text/` - Markdown файлы

### 3. Создание чанков (`run_chunking.py`)
- **Вход:** Markdown из S3 (`palladium-pdf-to-text`)
- **Процесс:**
  - Разбивает текст на чанки (300 токенов по умолчанию)
  - Добавляет перекрытие между чанками (50 токенов)
  - Сохраняет в JSON формате
- **Выход:**
  - S3 bucket `palladium-md-to-chunks/` - JSON с чанками

### 4. Индексация в Elasticsearch (`elastic_create.py`)
- **Вход:** JSON чанки из S3 (`palladium-md-to-chunks`)
- **Процесс:**
  - Создает эмбеддинги (multilingual-e5-large-instruct)
  - Индексирует в Elasticsearch
  - Настраивает kNN векторный поиск
- **Выход:**
  - Elasticsearch индекс с векторным поиском

### Схема данных

```
DOI → main.py → [articles.db + PDF в S3 (palladium-articles)]
                      ↓
                pdf_parser.py → [Markdown в S3 (palladium-pdf-to-text)]
                      ↓
                run_chunking.py → [JSON чанки в S3 (palladium-md-to-chunks)]
                      ↓
                elastic_create.py → [Elasticsearch индекс для векторного поиска]
```

### Пример полного запуска

```bash
# Шаг 1: Собираем статьи и PDF
python main.py --doi "10.48550/arXiv.2002.05202" --max-depth 1 --per-parent-limit 5

# Шаг 2: Парсим PDF в Markdown
python pdf_parser.py

# Шаг 3: Создаем чанки из Markdown
python run_chunking.py --mode s3

# Шаг 4: Индексируем в Elasticsearch
python elastic_create.py
```

---

## Детальное описание: Сбор статей (main.py)

Скрипт `main.py` - это краулер, который собирает научные статьи, начиная с одной "семенной" статьи (заданной через DOI). Он рекурсивно обходит граф цитирований, скачивает PDF-файлы статей (используя открытые источники и Sci-Hub) и сохраняет их в S3-совместимое хранилище (Yandex Object Storage), а метаинформацию сохраняет в локальную базу данных SQLite.

### Как работает сбор статей
...
**Работа без S3:** Если S3 не настроен или креды неправильные, скрипт **не упадет с ошибкой**. Он продолжит работу и сохранит:
- ✅ Метаданные статей в `articles.db`
- ✅ Связи между статьями
- ❌ PDF файлы не будут загружены

Это полезно, если вам нужна только информация о статьях (граф цитирований, метаданные), но не сами PDF.
...
### Старые arXiv DOI (10.48550/arXiv.*)

OpenAlex мигрировал многие старые arXiv статьи на новую систему DOI. Старые DOI формата `10.48550/arXiv.XXXX.XXXXX` могут не работать.

**Пример:**
- ❌ Старый (не работает): `10.48550/arXiv.1706.03762`
- ✅ Новый (работает): `10.65215/ne77pf66`

Скрипт автоматически обнаруживает старые arXiv DOI и выдает информативное сообщение с инструкциями.

**Решение:** Найдите актуальный DOI на [openalex.org](https://openalex.org) по названию статьи или arXiv ID.

**Примечание:** Некоторые более новые arXiv DOI с префиксом `10.48550/` всё ещё работают. Проблема касается в основном старых, высокоцитируемых статей.
